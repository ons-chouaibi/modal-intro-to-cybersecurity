...
int download_page(url_info *info, http_reply *reply) {
    ...
    int status = getaddrinfo(info->host, port, &hints, &servinfo);
    if (status != 0) {
    ...
    ssize_t bytes_sent = write(sockfd,http_request,strlen(http_request));
    if(bytes_sent<0){
    ...
    while(1){
        if (bytes_received+1000>buffer_size){
            buffer_size*=2;
        ...
        ssize_t n=recv(sockfd,buffer+bytes_received,buffer_size-bytes_received-1,0);
...
void write_data(const char *path, const char * data, int len) {
    ...
    if (written != len) {                         
        fprintf(stderr, "Warning: Only wrote %zu bytes out of %d\n", written, len);
    }
    fclose(file);
}
...
char *read_http_reply(struct http_reply *reply) {
    ...
    *status_line = '\0'; // Make the first line is a null-terminated string
    ...
    while (1) {
        char *line_end = next_line(current_pos, remaining_len);
        if (line_end == NULL) {
            fprintf(stderr, "Malformed HTTP response: no end of headers found\n");
            return NULL;
        }
        
        if (line_end == current_pos) {
            if ((status==301) && redirect_location != NULL) {
                while (*redirect_location == ' ') redirect_location++;
                
                fprintf(stderr, "Following redirect to: %s\n", redirect_location);
                
                url_info redirect_info;
                char *redirect_url = strdup(redirect_location);
                if (!redirect_url) {
                    fprintf(stderr, "Memory allocation error\n");
                    return NULL;
                }
                
                int parse_result = parse_url(redirect_url, &redirect_info);
                if (parse_result != 0) {
                    fprintf(stderr, "Failed to parse redirect URL: %s\n", parse_url_errstr[parse_result]);
                    free(redirect_url);
                    return NULL;
                }
                ...
                int download_result = download_page(&redirect_info, reply);
                free(redirect_url);
                if (download_result != 0) {
                    fprintf(stderr, "Failed to download from redirect location\n");
                    return NULL;
                }
        ...
        *line_end = '\0';  
        if ((status==301) && strncasecmp(current_pos, "Location:", 9) == 0) {
            redirect_location = current_pos + 9;
        }
        
        current_pos = line_end + 2;
        remaining_len = reply->reply_buffer_length - (current_pos - reply->reply_buffer);
    }
