...
{   
    ...
    // Check if we're using the "any" interface (LINUX_SLL format)
    if (strncmp(dev_name, "any", 3) == 0) {
        ethernet_header_size = 16;  // LINUX_SLL header size
    }

    // Log the packet data
    fprintf(logfile, "\n\n***********************Packet*************************\n");
    PrintData(buffer, header->len);
    
    // Get the IP Header part of this packet, using correct header size
    struct iphdr *iph = (struct iphdr*)(buffer + ethernet_header_size);
